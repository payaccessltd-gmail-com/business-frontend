    ...
    return (
        <div className='w-full flex flex-col items-center pb-[111px]'>
            ...
            <div className='flex flex-col items-center gap-6 w-full'>
                {
                    DescriptionData.map(({ id, title, subtitle, text, linkText, link, img, direction, }) => {
                        return <DescriptionCard key={id} title={title} subtitle={subtitle} text={text} linktext={linkText} link={link} img={img} direction={direction} />
                    })
                }
            </div>
        </div>
