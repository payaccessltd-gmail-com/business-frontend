...
export default function AuthenticationPage() {
    ...
    customPaging: (i: number) => (
      <div
        ...
        className={`h-[4px] rounded-[4px] transition-all .1s ease-in-out ${
          currentSlide === i ? "bg-[#F38020] w-[21px] translate-x-[-50%]" : "bg-[#D9D9D9] w-[5px]"
        }`}
      ></div>
    ),
  }
  return (
    <div className="flex flex-row w-full h-full ">
      <div className={`lg:flex hidden login-bg overflow-hidden relative w-1/2 h-full bg-primary/10 flex-col items-center justify-center`}>
      ...
      <div className="flex flex-col lg:w-1/2 w-full h-full items-center justify-center xl:px-[160px] lg:px-[100px] md:px-[120px] sm:px-[200px] px-[20px]">
        ...
        <div className="flex flex-row items-center gap-2 mt-4">
          ...
          <Link href="/registration" className="text-sm font-semibold">
            Sign Up
          </Link>
        </div>
      </div>
