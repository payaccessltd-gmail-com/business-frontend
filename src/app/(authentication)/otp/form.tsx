  ...
  const resendPasswordMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      if (responseData?.statusCode === "0") {
        ...
        if (typeof window) {
          router.push(`/otp?email=${email}&link=${responseData?.responseObject}`)
        }
      }
    },
    ...
    <Form {...otpForm}>
      ...
      >
                ...
                <div className="flex flex-row items-center gap-3">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      className="bg-[#FFFFFF] border text-center border-primary/20 border-solid h-12 w-12 rounded-[6px]"
                      // {...field}
                    />
                  ))}
                </div>
        ...
        <p className="text-[#1A1A1A] mt-6 text-[14px] text-center font-[400] leading-[145%]">
          Didn’t get the mail?{" "}
          <span className="text-primary font-[700] leading-normal cursor-pointer" onClick={(event) => handleResend(event)}>
            {loading1 ? "resending..." : "click here to resend."}
          </span>
        </p>
        <p className="text-[14px] font-[400] leading-[145%] text-[#000000] mt-[24px]">
          Resend code in{" "}
          <span className="text-[14px] font-[700] leading-normal text-[#CA6B1B]">{`${Math.floor(countdownTime / 60)}:${countdownTime % 60}`}</span>
        </p>
      </form>
    </Form>
  )
