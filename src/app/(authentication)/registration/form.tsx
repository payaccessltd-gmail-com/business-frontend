...
export default function RegistrationForm() {
  ...
  return (
    <Form {...registrationForm}>
      <form onSubmit={registrationForm.handleSubmit(onSubmit)} className="w-full rounded-lg bg-white pb-[50px] space-y-6 flex flex-col items-center">
        {/* <FormMessage> */}
        <div className="flex flex-row items-center w-fit h-fit">
          <div className="w-6 h-6 rounded-full bg-primary"></div>
          <div className={`h-[3px] w-[38px] bg-primary`}></div>
          <div className={`w-6 h-6 rounded-full border border-primary bg-[#D9D9D9]`}></div>
        </div>
        {/* </FormMessage> */}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="w-full">
              ...
              <FormControl>
                <Input type="password" icon="show" className="min-h-[48px]" placeholder="confirm Password" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
            ...
            <FormItem className="w-full">
              <FormControl>
                <div className="flex flex-row items-start gap-4">
                  <Checkbox checked={field.value} onCheckedChange={field.onChange as () => void} id="terms" className="border-border" />
                  <label htmlFor="terms" className="text-[14px] font-[400] leading-[145%] text-[#555]">
                    {`I consent to the collection and processing of my personal data in line with data regulations as described in `}
                    <Link className="text-primary/70 text-[16px] font-[600] underline" href="/">
                      Pay Access Policy
                    </Link>
                  </label>
                </div>
        ...
        <Button disabled={loading} className="mt-[32px] min-h-[48px] w-1/2 hover:bg-[#1D8EBB] hover:opacity-[0.4]" type="submit">
